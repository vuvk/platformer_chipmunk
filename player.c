...
void PlayerUpdate ()
{
    ...
    SPhysBody* physBody = player->physBody;

    // ограничиваем вращение игрока
    //physBody->body->a = DegToRad (180.0f);

    //
    PhysBodyUpdateState (physBody);
    ...
    if (physBody->body->v.y >  PLAYER_JUMP_HEIGHT)
        physBody->body->v.y =  PLAYER_JUMP_HEIGHT;
    /*if (physBody->body->v.y < -PLAYER_JUMP_HEIGHT)
        physBody->body->v.y = -PLAYER_JUMP_HEIGHT;*/
    ...
    center.y = -center.y;
    /* X */
    cameraPos.x = center.x - (WINDOW_WIDTH >> 1);
    if (cameraPos.x < 0.0f)
        cameraPos.x = 0.0f;
    uint16 maxCameraPosX = LEVEL_WIDTH * BLOCK_SIZE - WINDOW_WIDTH;
    if (cameraPos.x > maxCameraPosX)
        cameraPos.x = maxCameraPosX;
    /* Y */
    cameraPos.y = center.y - (WINDOW_HEIGHT >> 1);
    if (cameraPos.y < 0.0f)
        cameraPos.y = 0.0f;
...
void PlayerUpdateFrames ()
{
    ...
    {
        // движется?
        if (fabs(player->physBody->body->v.x) > BLOCK_SIZE/32.0f)
        {
            CreatureSetFrameRange (player, 2, 3);
        }
        else
        {
            // staying
            CreatureSetFrameRange (player, 1, 1);
        }
    }
}
